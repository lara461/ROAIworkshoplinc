...
The facilitator's own experience (at `/w/:workshopId`, after logging in with name + email) mirrors the admin's — a sidebar on desktop, a bottom tab bar on mobile — with three sections. The first time a facilitator logs in (per device), a short spotlight tour dims the screen and highlights the actual "My group" / "Workshop" / "Report" nav button for each step (switching the page behind it to match), so they see exactly what to tap, not just a description; it won't show again after that, but a **?** button next to the logo reopens it anytime.
