...
A **Reset progress** button (top of the tab, with a confirmation prompt) is meant for running a full dry-run with real facilitators — e.g. the day before the actual session — and then wiping everyone's work to start clean: every group goes back to Question 1, and all answers, board feedback, and closing reports are deleted. Groups, participants, and challenges (including which challenge each group already picked) are left completely untouched, and the workshop stays launched and ready to go — no need to re-launch it.
