...
5. **Generate challenges for all groups at once** — one button, grounded in each group's survey answers plus the Knowledge Base (if any). Each option is a real business decision (C-level/boardroom altitude) that simultaneously puts one of four themes to the test — frontline impact, trust & transparency, organizational values/culture, mission alignment — woven into the decision itself rather than tacked on separately; across the generated options, all four themes get exercised. Editable after generation. Picking which option a group works on is the **facilitator's call, on their own link** — the admin can edit wording but doesn't select for them. A **Download PDF** button exports every group's generated options (marking which one, if any, has been picked) into a single PDF — useful for someone to review them all offline before the workshop.
...
The facilitator's own experience (at `/w/:workshopId`, after logging in with name + email) mirrors the admin's — a sidebar on desktop, a bottom tab bar on mobile — with three sections. A **Public link** button (top-right, next to the ? on desktop; in the top bar on mobile) copies the public groups link (`/groups/:workshopId`) so the facilitator can quickly share it with their group if someone wants to follow along on their own phone. On **desktop**, the first time a facilitator logs in (per device), a short guided tour with a **live spotlight** covers just the essentials — the My group / Workshop / Report nav buttons, the Public link button, and — as the closing step — the ? button itself; the real element dims out the rest of the screen and gets highlighted, with the tooltip positioned to stay clear of it. Everything else is left to be self-explanatory rather than walked through step by step. It won't show again after that, and the **?** button reopens it anytime. **On mobile there's no tour at all** — the layout there is simple enough not to need one, so the ? button and its logic are skipped entirely; the AI briefing box and the "Your challenge" box collapse into tap-to-expand accordions there too, so they don't push the rest of the page below the fold.

- **My group** — an AI-generated briefing on the group's composition (from the other members' pre-work survey answers, not the facilitator's own — where they broadly stand on AI maturity, where views converge or diverge), auto-generated once, plus each of those members shown as an always-open card (name, email, survey answers) — nothing to click through, all visible at a glance.
- **Workshop** — a short line on how to use the section, then the challenge itself in its own highlighted box, then the actual activity navigated with step tabs (Question 1 / Board Feedback / 30/60/90 actions). Steps not yet reached are locked, but once reached the facilitator can tab back to review earlier ones — nothing is hidden once it's been done.
...
1. **Pick the challenge** — right after launch, the facilitator picks which of the generated options their group works on. Then **Question 1** — the facilitator writes and saves the group's initial answer, and can keep revising it right up until they push it to the board — pushing is what locks it in, not saving.
2. **C-level board challenge + revised answer** — the facilitator explicitly pushes their group's answer to the board with a button (no admin action needed); four board members react. They're shown anonymously, with no title or name attached — just four reactions, so the focus stays on what's said, not who said it. Behind the scenes the AI does pick four contextually-fitting executive titles (CFO, Chief People Officer, General Counsel, Chief Risk Officer, etc., varying each time) to keep the four reactions genuinely distinct, it just isn't surfaced in the UI. This workshop is specifically about the human side of AI adoption, so every reaction fuses a concrete business angle (cost, risk, feasibility, execution) with one of four themes — frontline impact, trust & transparency, organizational values/culture, mission alignment — never as a business point and a "by the way, also think about the humans" tacked on separately, and grounded in the workshop's own **Knowledge Base** where one is provided (not a generic framework). The tone aims to prompt real reflection, not to simulate a hostile pile-on.
...
The admin's Workshop tab is a live 3-column board (Question 1 / Board Feedback / 30/60/90 actions) — every group appears as a card in whichever column matches its current activity, so you can see at a glance who's ahead and who's behind. Tap a group's card to open a popup with everything it's done so far — **Challenge**, **Their solution**, **Board feedback**, **Reviewed solution**, **30/60/90-day actions** — plus **← Move back / Move forward →** buttons to manually move that group to a different step (forward to unstick a facilitator, or back to reopen an earlier one for editing). Moving back doesn't delete anything by itself — the facilitator can resubmit the same content and nothing downstream changes, or they can genuinely edit it, in which case whatever depended on the old content (the board's feedback, the revised answer, the 30/60/90 actions — whichever comes after the step that changed) is cleared and has to be redone, since it was grounded in content that no longer exists. A manual "regenerate board challenge" fallback button is also available in case the automatic one needs a redo.

A **Reset progress** button (top of the tab, with a confirmation prompt) is meant for running a full dry-run with real facilitators — e.g. the day before the actual session — and then wiping everyone's work to start clean: every group goes back to picking a challenge (their currently-picked one gets un-selected, all options go back to "option"), and all answers, board feedback, and closing reports are deleted. Participants and the generated challenge options themselves are left completely untouched — only which one each group had picked gets cleared, since picking a challenge is genuinely the first thing a group does. The workshop stays launched and ready to go — no need to re-launch it.
...
src/components/PublicGroupsView.tsx — Public, no-login: each group's progress shown via a simple status tag; tap a group to see everything they've written so far — challenge, initial answer, board feedback, revised answer, 30/60/90 actions — each in its own colored-title callout box for easy scanning. Only the available phases show up as tabs (icon + label), one at a time, so nobody scrolls through a wall of content: a fixed bottom bar on mobile (app-style), a horizontal tab row up top on desktop.
...
src/challengesPdf.ts                — Generates every group's challenge options as a downloadable PDF, client-side (lazy-loaded, not in the main bundle)
