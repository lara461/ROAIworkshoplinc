...
The facilitator's own experience (at `/w/:workshopId`, after logging in with name + email) mirrors the admin's — a sidebar on desktop, a bottom tab bar on mobile — with three sections. A **Public link** button (top-right, next to the ? on desktop; in the top bar on mobile) copies the public groups link (`/groups/:workshopId`) so the facilitator can quickly share it with their group if someone wants to follow along on their own phone. The first time a facilitator logs in (per device), a guided tour walks them through everything with a **live spotlight** — the actual button or area dims out the rest of the screen and gets highlighted, with the tooltip positioned to stay on-screen based on which third of the viewport the target sits in (side callout when there's room, otherwise stacked above/below and aligned left, center, or right to match). This covers the nav buttons, the AI briefing box, the workshop's step tabs, the Question-1 answer box, the C-level board's feedback area, the report document, and — as the closing step — the Public link and ? buttons; a couple of purely conceptual steps (how the 3 activities generally work) stay as plain illustrated cards since they're not tied to one specific spot on screen. Background scrolling is frozen for the whole tour, and each step scrolls its own target into view automatically — nothing can drift out from under the spotlight because the page happened to be scrolled somewhere else. Skipping or closing early always lands on the final "you can always find this again" reminder first, so everyone sees it regardless of how far they got. It won't show again after that, but the **?** button reopens it anytime.
