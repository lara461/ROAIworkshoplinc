...
The facilitator's own experience (at `/w/:workshopId`, after logging in with name + email) mirrors the admin's — a sidebar on desktop, a bottom tab bar on mobile — with three sections. The first time a facilitator logs in (per device), a 10-step guided tour walks them through everything, mixing two styles on purpose: **live spotlight** on the stable chrome that's always there regardless of workshop state (the ? button, and the My group / Workshop / Report nav buttons, plus the AI briefing box) — the real button dims out the rest of the screen and gets highlighted; and **plain illustrated cards** for anything that depends on where the workshop actually is right now (how to navigate the step tabs, writing the first answer, the board's auto-generated response, editing the report) — since that content might not even exist yet for a given group, spotlighting it live would be unreliable. It won't show again after the first time, but the **?** button (floating top-right on desktop, top bar on mobile) reopens it anytime.
