...
The facilitator's own experience (at `/w/:workshopId`, after logging in with name + email) mirrors the admin's — a sidebar on desktop, a bottom tab bar on mobile — with three sections. A **Public link** button (top-right, next to the ? on desktop; in the top bar on mobile) copies the public groups link (`/groups/:workshopId`) so the facilitator can quickly share it with their group if someone wants to follow along on their own phone. The first time a facilitator logs in (per device), an 11-step guided tour walks them through everything, mixing two styles on purpose: **live spotlight** on the stable chrome that's always there regardless of workshop state (the My group / Workshop / Report nav buttons, the AI briefing box, the Public link button, and — as the closing step — the ? button itself) — the real element dims out the rest of the screen and gets highlighted, with the tooltip positioned to stay on-screen wherever the target sits (side, above, or below, whichever fits); and **plain illustrated cards** for anything that depends on where the workshop actually is right now (how to navigate the step tabs, writing the first answer, the board's auto-generated response, editing the report) — since that content might not even exist yet for a given group, spotlighting it live would be unreliable. Skipping or closing early always lands on the final "you can always find this again" reminder first, so everyone sees it regardless of how far they got. It won't show again after that, but the **?** button reopens it anytime.
