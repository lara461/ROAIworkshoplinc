...
The facilitator's own experience (at `/w/:workshopId`, after logging in with name + email) mirrors the admin's — a sidebar on desktop, a bottom tab bar on mobile — with three sections. A **Public link** button (top-right, next to the ? on desktop; in the top bar on mobile) copies the public groups link (`/groups/:workshopId`) so the facilitator can quickly share it with their group if someone wants to follow along on their own phone. On **desktop**, the first time a facilitator logs in (per device), a guided tour walks them through everything with a **live spotlight** — the actual button or area dims out the rest of the screen and gets highlighted, with the tooltip positioned to stay clear of the highlighted content (side callout when there's room, otherwise above/below, or pinned to the bottom of the screen if the target itself is too large to fit a tooltip around). This covers the nav buttons, the AI briefing box, the workshop's step tabs, the Question-1 answer box, the C-level board's feedback area, the report document, and — as the closing step — the Public link and ? buttons; a couple of purely conceptual steps (how the 3 activities generally work) stay as plain illustrated cards since they're not tied to one specific spot on screen. Background scrolling is frozen for the whole tour, and each step scrolls its own target into view automatically. Skipping or closing early always lands on the final "you can always find this again" reminder first. It won't show again after that, and the **?** button reopens it anytime. **On mobile there's no tour at all** — the layout there is simple enough not to need one, so the ? button and its logic are skipped entirely; the AI briefing box and the "Your challenge" box collapse into tap-to-expand accordions there too, so they don't push the rest of the page below the fold.
